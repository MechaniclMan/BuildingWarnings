...
//Building Warnings Rencorner fork. Thank you Unstoppable!
...
const char BuildingWarningsGameFeatureClass::Sounds[4][2][50] = {
        {
...
DASettingsClass* BuildingWarningsGameFeatureClass::INI = 0;

void BuildingWarningsGameFeatureClass::Init()
{
    //Allow players to dsiable building warning sounds
    ...
    Console_Output("%sVersion %.2f for Rencorner, built at %s\n", DA::Get_Message_Prefix(), Version, CompileTime);
}

void BuildingWarningsGameFeatureClass::Settings_Loaded_Event()
{
    INI->Reload();
...
void BuildingWarningsGameFeatureClass::SendStateMessage(FormatParams *Params)
{
    if (!Is_Timer(ColorCast(Params->State), Commands->Get_ID(Params->Building)))
    {
        ...
        case WarnState::WARNSTATE_75:
            ...
            if(ShowEnemy) Create_2D_Sound_Enabled_Team(PTTEAM(Params->Team), Sounds[0][Params->Team]);
            break;
        case WarnState::WARNSTATE_50:
            DA::Create_2D_Sound_Team(Params->Team, Sounds[1][Params->Team]);
            if (ShowEnemy) Create_2D_Sound_Enabled_Team(PTTEAM(Params->Team), Sounds[1][Params->Team]);
            break;
        case WarnState::WARNSTATE_25:
            DA::Create_2D_Sound_Team(Params->Team, Sounds[2][Params->Team]);
            if (ShowEnemy) Create_2D_Sound_Enabled_Team(PTTEAM(Params->Team), Sounds[2][Params->Team]);
            break;
        case WarnState::WARNSTATE_REPAIRING:
            Create_2D_Sound_Enabled_Team(PTTEAM(Params->Team), Sounds[3][Params->Team]);
            break;
        }
...
void BuildingWarningsGameFeatureClass::FormatStateMessage(FormatParams *Params)
{
...
void BuildingWarningsGameFeatureClass::Create_2D_Sound_Enabled_Team(int Team, StringClass Sound)
{
	for (SLNode<cPlayer> *z = Get_Player_List()->Head();z;z = z->Next()) 
	{
		...
		if (Player->Is_Active() && Get_Player_Data(Player)->WarningSounds ) 
		{
...
bool BuildingWarningsGameFeatureClass::Chat_Command_Event(cPlayer *Player,TextMessageEnum Type,const StringClass &Command,const DATokenClass &Text,int ReceiverID)
{
	...
	{
		Get_Player_Data(Player)->WarningSounds = true;
	...
	{
		Get_Player_Data(Player)->WarningSounds = false;
		DA::Page_Player(Player,"You will no longer hear building warning sounds.");
	}
	return true;
}

Register_Game_Feature(BuildingWarningsGameFeatureClass, "Building Warnings", "EnableBuildingWarnings", 0);